...
<Callout type="info" title="为什么需要意向锁: 快速判断表中是否有记录被加记录锁">
...
where (salary, leader_id) = (
  select salary, leader_id from t_emp where name = "yukino"
)

-- 表子查询: 子查询的结果为多行多列 (一张表)
select * from t_emp
where (job, salary) in (
  select job, salary from t_emp where name = "yukino" or name = "yukino2"
)
...
explain select name, age from users where name = 'yukino' and age = 22;
-- using index condition: 查询时使用了索引, 但是需要回表查询
explain select * from users where name = 'yukino' and age = 22;
```
...
-- 联合索引有效
select * from users where name = 'yukino' and age = 22 and gender = 1;
select * from users where name = 'yukino' and age = 22;
select * from users where name = 'yukino';
...
select * from users where name = 'yukino' and gender = 1;
```
...
<Tabs items={['Demo 1', 'Demo 2', 'Demo 3', 'Demo 4']}>
<Tab value="Demo 1">
