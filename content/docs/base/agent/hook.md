...
- ~/.yukino/config.yaml
- ${projectRoot}/.yukino/config.yaml
- ${projectRoot}/.yukino/config.local.yaml
...
- startup: Yukino 启动时触发
- shutdown: Yukino 退出时触发
- error: Yukino 发生错误时触发
- compact: 上下文压缩时触发
...
hooks:
  ...
  - id: project-context
    ...
    action:
      type: prompt
      prompt: |
        - Tech Stack: TypeScript, React, Ink, Zod, Vitest, ESLint, oxfmt, pnpm workspace deps: `@yukino.js/glob-addon`, `@yukino.js/glob-wasm`.
