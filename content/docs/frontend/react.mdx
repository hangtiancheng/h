...
```tsx
function App() {
  const htmlSnippet = '<div style="color: skyblue">yukino</div>';
  // 类似 v-html
...
<Tab value="@/hooks/useLocalStorage.ts">
...
export default function App() {
  const [inputVal, setInputVal] = useState("yukino");
  const [user, setUser] = useState({ name: "yukino" });
  return (
    <>
      ...
      <button
        onClick={() =>
          setUser((draft) => {
            ...
            draft.push({ name: `yukino${nextAge}`, age: nextAge });
          })
        }
      >
        Push Next
