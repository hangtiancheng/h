...
> See "$HOME/github/yukino-sentry/sentry"

属性约定

| 属性                 | 含义       | 示例                                 |
| -------------------- | ---------- | ------------------------------------ |
| `yukino-sentry-ev`   | 事件 ID    | `yukino-sentry-ev="toggle-language"` |
| `yukino-sentry-msg`  | 事件描述   | `yukino-sentry-msg="切换语言"`       |
| `yukino-sentry-view` | 元素标记   | `yukino-sentry-view="toggle-button"` |
| `yukino-sentry-*`    | 自定义参数 | `yukino-sentry-lang="en"`            |
...
export function getDeclarativeClickData(
  ...
  // 找到路径中, 第一个携带 yukino-sentry-ev/msg/view 属性的元素
  ...
  return {
    ev: getEventId(fallbackPath), // 优先级: yukino-sentry-ev > title > yukino-sentry-view > 标签名
    msg: getMessage(trackingTarget), // 优先级: yukino-sentry-msg > title > textContent > aria-label > 标签名
    triggerPageUrl: location.href,
    x: left + scrollLeft,
    y: top + scrollTop,
    params: getParams(fallbackPath), // 收集 yukino-sentry-* 自定义参数 (ev/msg/view 除外)
