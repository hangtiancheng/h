...
```ts
const refObj = ref(1);
const refObj2 = ref({ name: "yukino", age: 1 });
const reactiveObj = reactive({ name: "yukino", age: 1 });
```
...
<Callout type="info" title="ref/reactive 深层响应式">
...
<Tabs items={['写法 1', '写法 2', '写法 3', '写法 4']}>
<Tab value="写法 1">
...
<Tabs items={['ChildDemo.vue', 'ParentDemo.vue', 'ParentDemo.vue动态插槽']}>
<Tab value="ChildDemo.vue">
...
const authList = [
  "yukino:item:create",
  "yukino:item:update" /** 'yukino:item:delete' */,
];
...
<template>
  ...
  <div>
    ...
    {{ $encoding.jsonMarshal({ name: "yukino", age: 23 }) }}
  </div>
</template>
...
console.log(app?.proxy?.$encoding.jsonMarshal({ name: "yukino", age: 23 }));
</script>
```
...
<Tab value="pnpm build && pnpm preview">
