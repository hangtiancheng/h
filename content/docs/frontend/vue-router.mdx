...
const routeJumpByQuery = (user: User) => {
  router.push({
    ...
    // query: url 查询参数
    // http://localhost:5173/register?id=1&name=yukino&age=23
    query: user,
    state: user,
  });
};
...
<Tab value="@/views/RegisterView.vue">
...
<Tabs items={['@/router/index.ts', '@/views/HomeView.vue']}>
<Tab value="@/router/index.ts">
...
<Tab value="@/components/ProgressBar.vue">
