...
- Chrome DevTools Network: TTFB、瀑布图、请求时序、请求优先级、资源体积
- Chrome DevTools Performance: longtask、火焰图、主线程阻塞、JS 执行耗时
...
|          | EventSource                                                                       | fetch + response.body.getReader                                           |
| -------- | --------------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| 协议     | 强制 SSE (Content-Type: text/event-stream, 服务器按 id:/event:/data:/retry: 分帧) | 任意 Content-Type                                                         |
| 方法     | 只能使用 GET, 不能携带请求体、不能自定义请求头                                    | 任意 HTTP 方法, 可以携带请求体、可以自定义请求头, 例如 Authorization      |
| 断线重连 | 浏览器自动重连, 返回 Last-Event-ID 给服务器                                       | 手动重连                                                                  |
| 分帧     | 浏览器自动分帧, es.onmessage 可以拿到完整消息                                     | `reader.read() `返回的是字节 chunk, 一个 chunk 可能包含部分事件或多个事件 |
| 错误信息 | es.onerror 没有状态码, 没有响应体, 拿不到错误信息                                 | 可以读 response.headers, response.status; 区分 401/429/5xx 和流中断       |
| 取消     | 只能使用 `es.close()`                                                             | 可以使用 AbortController, 可以设置超时, 可以传播 signal                   |
| 连接占用 | HTTP/1.1 同一个域名有 6 个连接上限, SSE 长期占用一个                              | 也占用一个, 可以手动释放                                                  |
...
- secure: `secure=true` 时
  - 只有 HTTPS 请求时, 浏览器才会携带 cookie 并发送到服务器
  - HTTP 请求时, 浏览器不会携带 cookie 并发送到服务器
...
<Tab value="cors/2024/index.html">
...
<Tab value="cors/2025/index.html">
...
<Tab value="frontend (localhost:5500)">
...
<Tab value="backend (localhost:8080)">
...
- access token 放内存, refresh token 放 httponly cookie
- access token 短期, 每次请求携带
- refresh token 长期, token 续期时使用, 可以服务端吊销; access token 过期时, 使用 refresh token 刷新 access token; 优点是即使 access token 泄漏, 攻击窗口也很短; refresh token 可以通过重放检测发现

| 身份认证方法                                          | 优点                                                         | 缺点                   |
| ----------------------------------------------------- | ------------------------------------------------------------ | ---------------------- |
| 内存变量                                              | 最安全, 页面刷新即丢失                                       | 页面刷新后需要重新获取 |
| secure=true; httponly=true; SameSite=Strict 的 cookie | https、同源请求才携带 cookie, JS 不能读写 cookie; 可以防 XSS | 需要配合 CSRF          |
