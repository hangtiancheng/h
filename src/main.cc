...
class Response {
 public:
  ...
  Response& type(const std::string& mime) {
 ...
 private:
  void finish(const std::string& body) {
    ...
    if (!headers_.count("Connection")) headers_["Connection"] = "close";
    for (const auto& kv : headers_) head << kv.first << ": " << kv.second << "\r\n";
    head << "\r\n";
 ...
 public:
  ...
  void use(Handler h) {
    middlewares_.push_back(
        [h](Request& req, Response& res, Next next) {
          h(req, res);
 ...
 private:
  ...
  void handle(int fd) {
    ...
    try {
      ...
      if (!res.finished()) res.status(500).send("Internal Server Error");
    }
    ::close(fd);
  }
  ...
  void dispatch(Request& req, Response& res) {
    ...
    std::function<void(size_t)> run = [&](size_t idx) {
      if (idx >= chain.size() || res.finished()) return;
