...
<Tabs>
  <TabItem label="GrandparentDemo.vue">
...
```
  ...
  <TabItem label="ParentDemo.vue动态插槽">
```vue filename="ParentDemo.vue动态插槽"
<script lang="ts" setup>
import { ref } from "vue";
import ChildDemo from "./ChildDemo.vue";
...
<Tabs>
  <TabItem label="public/data.json">
...
```
  ...
  <TabItem label="ChildSkeleton.vue">
...
```shell filename=".env.development"
...
```
  ...
  <TabItem label="pnpm build && pnpm preview">
...
```
  ...
  <TabItem label="d2vue-btn.ce.vue">
...
<Tabs>
  <TabItem label="frontend (localhost:5500)">
...
```
  ...
  <TabItem label="backend (localhost:8080)">
