...
git config --global user.email 'hangtiancheng@qq.com' && \
git config --global core.autocrlf false &&           \
git config --global credential.helper store &&       \
git config --global init.defaultBranch main &&       \
git config --global core.filemode false

ssh-keygen -t rsa -C 'hangtiancheng@qq.com'

ssh-keygen -t rsa -C 'yukinohangtiancheng@gmail.com' -f ~/.ssh/id_rsa_github
ssh-keygen -t rsa -C 'yukinohangtiancheng@gmail.com' -f ~/.ssh/id_rsa_github-2
...
```shell
git diff [--stat] origin/feat..feat/tiancheng/layout-v2
```
